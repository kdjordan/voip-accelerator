    ...
    >
      <div class="flex items-start">
        <div class="flex-shrink-0 mr-3">
          <!-- Info icon -->
          <svg v-if="type === 'info'" class="h-6 w-6 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          
          <!-- Success icon -->
          <svg v-else-if="type === 'success'" class="h-6 w-6 text-green-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          
          <!-- Warning icon -->
          <svg v-else-if="type === 'warning'" class="h-6 w-6 text-yellow-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          
          <!-- Error icon -->
          <svg v-else-if="type === 'error'" class="h-6 w-6 text-red-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button @click="dismiss" class="flex-shrink-0 ml-2 text-white text-opacity-70 hover:text-opacity-100">
...
function handleStorageStrategyChange(event: CustomEvent) {
  ...
  if (newType === 'indexeddb' && reason === 'memory-pressure') {
    ...
    isVisible.value = true;
    props.title = 'Storage Strategy Changed';
    props.message = 'Due to high memory usage, the application has automatically switched to IndexedDB storage. This may impact performance.';
    props.type = 'warning';
    startDismissTimer();
  }
}
...
watch(() => [props.message, props.title, props.type], () => {
  if (isVisible.value) {
    if (dismissTimer.value) {
      clearTimeout(dismissTimer.value);
    }
