...
export interface MetroAreaOption {
  key: string;
  displayName: string;
  population: number;
  areaCodes: string[];
...
// Utility: Convert metro key to display name (e.g., "new-york-ny" -> "New York, NY")
export function getMetroDisplayName(key: string): string {
  if (!key) return '';
  // Replace dashes with spaces, capitalize words, and add comma before state
...
// Export a list of metro area options for dropdowns
export const metroAreaOptions: MetroAreaOption[] = Object.entries(metroToNpaMap).map(
  ([key, value]) => ({
    key,
    displayName: getMetroDisplayName(key),
