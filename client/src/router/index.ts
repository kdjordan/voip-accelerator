...
export const routes: RouteRecordRaw[] = [
  {
    ...
    meta: { public: true, marketing: true },
  },
  {
    path: '/home',
    redirect: '/',
  ...
  // --- Auth Routes ---
  {
    ...
    meta: { requiresAuth: false, hideWhenAuthed: true, public: true, marketing: true },
  },
  {
    path: '/signup',
    name: 'SignUp',
...
export function installRouterGuards(router: Router): void {
  router.beforeEach(async (to, from, next) => {
    ...
    const hidesWhenAuthed = to.matched.some((record) => record.meta.hideWhenAuthed);
    const requiresAdmin = to.matched.some((record) => record.meta.requiresAdmin);

    if ((requiresAuth || requiresAdmin || hidesWhenAuthed) && !userStore.getAuthIsInitialized) {
