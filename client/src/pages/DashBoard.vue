...
function getModuleForTable(tableName: string): string {
  ...
  if (lowerTableName.includes('lerg') || lowerTableName.includes(DBName.LERG.toLowerCase())) {
    return 'LERG';
  }

  // Check for Rate Sheet tables
  if (
    lowerTableName.includes('rate') ||
    ...
    if (azRecords === 0 && (azStore as any).$state && (azStore as any).$state.inMemoryData) {
      Object.entries((azStore as any).$state.inMemoryData).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          ...
          console.log(
            `Found AZ memory data in $state.inMemoryData.${key}: ${value.length} records`
          );
        }
      });
    }
      ...
      if (inMemoryData && typeof inMemoryData === 'object') {
        Object.entries(inMemoryData).forEach(([key, value]) => {
          if (Array.isArray(value)) {
            azRecords += value.length;
            console.log(
              `Found AZ memory data in getInMemoryData().${key}: ${value.length} records`
            );
          }
        });
      }
        ...
        if (Array.isArray(value) && value.length > 0) {
          ...
          for (const nestedKey in value) {
            ...
            if (Array.isArray(nestedValue) && nestedValue.length > 0) {
              ...
              console.log(
                `Found AZ memory data in ${key}.${nestedKey}: ${nestedValue.length} records`
              );
            }
          }
        }
    ...
    if (usRecords === 0 && (usStore as any).$state && (usStore as any).$state.inMemoryData) {
      Object.entries((usStore as any).$state.inMemoryData).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          ...
          console.log(
            `Found US memory data in $state.inMemoryData.${key}: ${value.length} records`
          );
        }
      });
    }
      ...
      if (inMemoryData && typeof inMemoryData === 'object') {
        Object.entries(inMemoryData).forEach(([key, value]) => {
          if (Array.isArray(value)) {
            usRecords += value.length;
            console.log(
              `Found US memory data in getInMemoryData().${key}: ${value.length} records`
            );
          }
        });
      }
        ...
        if (Array.isArray(value) && value.length > 0) {
          ...
          for (const nestedKey in value) {
            ...
            if (Array.isArray(nestedValue) && nestedValue.length > 0) {
              ...
              console.log(
                `Found US memory data in ${key}.${nestedKey}: ${nestedValue.length} records`
              );
            }
          }
        }
    ...
    if ((rateSheetStore as any).$state && (rateSheetStore as any).$state.inMemoryData) {
      Object.entries((rateSheetStore as any).$state.inMemoryData).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          ...
          console.log(
            `Found RateSheet memory data in $state.inMemoryData.${key}: ${value.length} records`
          );
        }
      });
    }
      ...
      if (inMemoryData && typeof inMemoryData === 'object') {
        Object.entries(inMemoryData).forEach(([key, value]) => {
          if (Array.isArray(value)) {
            rateSheetRecords += value.length;
            console.log(
              `Found RateSheet memory data in getInMemoryData().${key}: ${value.length} records`
            );
          }
        });
      }
        ...
        if (Array.isArray(value) && value.length > 0) {
          ...
          for (const nestedKey in value) {
            ...
            if (Array.isArray(nestedValue) && nestedValue.length > 0) {
              ...
              console.log(
                `Found RateSheet memory data in ${key}.${nestedKey}: ${nestedValue.length} records`
              );
            }
          }
        }
...
function getStoreRecordCount(store: any): number {
  try {
    ...
    // For LERG store specifically, use the totalNPAs value if available
    if (store.stats && typeof store.stats.totalNPAs === 'number') {
      return store.stats.totalNPAs;
    }
...
async function getIndexedDbTables(): Promise<DatabaseTable[]> {
  ...
  try {
    ...
    try {
      ...
      if (totalAzRecords > 0) {
        console.log(
          `Found ${indexedDbAzTables.length} AZ tables in IndexedDB with ${totalAzRecords} total records`
        );
        tables.push({
          name: DBName.AZ,
    ...
    try {
      ...
      if (totalUsRecords > 0) {
        console.log(
          `Found ${indexedDbUsTables.length} US tables in IndexedDB with ${totalUsRecords} total records`
        );
        tables.push({
          name: DBName.US,
    ...
    if ((lergStore as any).stats?.totalNPAs > 0) {
      console.log(
        `Found LERG data in IndexedDB with ${(lergStore as any).stats.totalNPAs} records`
      );
      tables.push({
        name: DBName.LERG,
        count: (lergStore as any).stats.totalNPAs,
        storage: 'indexeddb' as const,
      });
    } else {
    ...
    try {
      ...
      const totalRateSheetRecords = indexedDbRateSheetTables.reduce(
      ...
      if (totalRateSheetRecords > 0) {
        console.log(
          `Found ${indexedDbRateSheetTables.length} Rate Sheet tables in IndexedDB with ${totalRateSheetRecords} total records`
        );
        tables.push({
          name: DBName.RATE_SHEET,
...
onMounted(async () => {
  ...
  console.log(
    'Note: az_rate_deck_db-storage and us_rate_deck_db-storage are implementation details'
  );
