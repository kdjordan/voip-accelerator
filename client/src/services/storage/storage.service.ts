...
export class StorageService<T = any> {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error(`[StorageService] Failed to initialize storage for database: ${this.dbName}`, err);
  ...
  public async getData(tableName: string): Promise<T[]> {
    try {
      ...
      if (storageConfig.enableLogging) {
        console.log(`[StorageService] Retrieved ${data?.length || 0} records from table: ${tableName}`);
      }
  ...
  public async listTables(): Promise<Record<string, number>> {
    try {
      ...
      if (storageConfig.enableLogging) {
        console.log(`[StorageService] Listed ${Object.keys(tables).length} tables for database: ${this.dbName}`);
      }
  ...
  public async changeStrategy(newStrategy: StorageStrategy<T>): Promise<void> {
    ...
    for (const { tableName } of tablesResult) {
      ...
      if (data) {
        ...
        if (storageConfig.enableLogging) {
          console.log(`[StorageService] Collected ${data.length} records from table ${tableName} for migration`);
        }
      }
    }
    ...
    for (const tableName of Object.keys(dataToMigrate)) {
      if (dataToMigrate[tableName].length > 0) {
        ...
        if (storageConfig.enableLogging) {
          console.log(`[StorageService] Migrated ${dataToMigrate[tableName].length} records to table ${tableName} in new strategy`);
        }
      }
    }
