...
import { DBNameType, DBName } from '@/types/app-types';
...
export async function forceRefreshStrategies(): Promise<void> {
  ...
  try {
    // Load AZ strategy
    const azStrategy = await getStorageStrategy(DBName.AZ);
    await azStrategy.initialize();
    console.log('[Storage Factory] Refreshed AZ strategy');

    // Load US strategy
    const usStrategy = await getStorageStrategy(DBName.US);
  ...
  const checkMemoryUsage = () => {
    ...
    if ('memory' in performance) {
      ...
      if (storageConfig.enableLogging) {
        console.debug(
          `[Memory Monitor] Current usage: ${usedHeapSizeMB.toFixed(2)}MB / Threshold: ${
            storageConfig.memoryThresholdMB
          }MB / Auto-fallback: ${
            storageConfig.autoFallbackOnMemoryPressure ? 'Enabled' : 'Disabled'
          }`
        );
      }
      ...
      if (
        !storageConfig.autoFallbackOnMemoryPressure ||
      ...
      if (usedHeapSizeMB > storageConfig.memoryThresholdMB) {
        console.warn(
          `Memory usage (${usedHeapSizeMB.toFixed(2)}MB) exceeded threshold (${
            storageConfig.memoryThresholdMB
          }MB). Switching to IndexedDB storage.`
        );
