...
export class DexieStrategy<T> extends BaseStorageStrategy<T> {
  ...
  // Always enable logging for debugging purposes
  ...
  async initialize(): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to initialize DexieJS database: ${
  ...
  async storeData(tableName: string, data: T[]): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i += chunkSize) {
        ...
        if (this.enableLogging && i + chunkSize < data.length) {
          console.log(
            `[DexieStrategy] Stored chunk ${i / chunkSize + 1} of ${Math.ceil(
      ...
      throw new Error(
        `Failed to store data in DexieJS: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async getData(tableName: string, options?: DataRetrievalOptions<T>): Promise<T[] | null> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get data from DexieJS: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async removeData(tableName: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to remove data from DexieJS: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  async clearAllData(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to clear all data from DexieJS: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  async getCount(tableName: string): Promise<number> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get count from DexieJS: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  async listTables(): Promise<{ tableName: string; recordCount: number }[]> {
    ...
    try {
      ...
      throw new Error(
        `Failed to list tables from DexieJS: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  private async ensureTableExists(tableName: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to create table in DexieJS: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
