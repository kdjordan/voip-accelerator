...
- **Active tasks:** **F ready to dispatch** (`feat/rg-average`, worktree `../va-wt-rg-average` @ `4f45d88`, cmux tab — owner dispatches) — Average→Blend redesign (depth+mode), test-first, full spec in [[rate-gen-rework]]. G ✅ merged (`4f45d88`, scenarios+sample now persist across tab nav; 114 unit green). Worker does NOT run dev server (owner's :5173 up; auth breaks on other ports).
- **Pre-merge follow-ups (owner-requested, before `main` merge):** G (scenario persistence — running, cmux). **G2** = "Tweak & re-run" recall on Generated Decks tab (depends on G). **F = Average/Blend redesign — DESIGN LOCKED (/grill-me 2026-05-28); dispatch AFTER G merges** (F & G both touch Scenario model + RateGenSimulation → would conflict; sequence, don't parallelize). F spec: strategy becomes `{depth, mode: take|blend}`; blend = avg of cheapest `depth` rates per-jurisdiction; available w/ 2 decks; gate depth by deck count + per-prefix fallback to `min(depth, available)`; blended provider = set-of-contributors (joined names in Route CSV; win-rate→"participation"); remove old standalone 'Average'. Test-first. Full spec in [[rate-gen-rework]] memory.
