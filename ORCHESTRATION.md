...
**⏳ 3 HELD follow-ups (owner's call):** (a) manual Coolify **DEPLOY** (next deploy ships xlsx + the hand-off arc — tag it); (b) **unify the 3 progress indicators** (Analyzer `UploadProgressIndicator` / Adjuster `RealTimeProgressIndicator` / Composition Studio `RateGenProgressIndicator`) into one shared component; (c) **PDF rebrand** (`pricing-audit-pdf.ts` + `rate-gen-summary-pdf.ts` still emerald → Switchboard).
