...
**✅ XLSX upload+export SHIPPED to `origin/main` = `6a3b7db` (pushed 2026-05-29, NOT deployed — owner HOLDING deploy). Board idle.** Accept/export `.xlsx` across Analyzer/Adjuster/Composition Studio (A-Z + admin-LERG deferred). ADR-0010; pinned `read-excel-file@9.0.9` + `write-excel-file@4.0.6` (GuardDog-vetted). Foundation `utils/tabular-io.ts` (+ `xlsx-parse.worker.ts`, `xlsx-cell.ts`, `export-format.ts`, `FormatToggle.vue`). All 4 slices + the fixes (worker-parse [read-excel-file ESM doesn't auto-worker — froze main thread], single-parse-reuse cache, preview cap, real progress, rate-gen band+metadata-spinner, single-zip Export Package restore) merged + verified live (mixed CSV+XLSX → generate → promote-to-Pricing-Studio). Bundle gate held (no unzipper/archiver-node in dist). Full detail = auto-memory [[xlsx-upload-export]].

**⏳ HELD follow-ups (owner's call):** (a) manual Coolify **DEPLOY** (next deploy ships xlsx + the hand-off arc + (b) unify-progress — tag it); (c) **PDF rebrand** (`pricing-audit-pdf.ts` + `rate-gen-summary-pdf.ts` still emerald → Switchboard).

**✅ (b) UNIFY PROGRESS INDICATORS — MERGED to `main` + PUSHED (`origin/main` = `6909891`), deploy HELD.** Owner gut-checked all 3 surfaces + the provider card at :5173 ("looks good"). FF-merged from `feat/unify-progress-indicator`; 4 commits: `85fd043` component unify, `a93b1fd` Adjuster curve fix, `00bd202` shared write-progress helper + Studio freeze-at-90 fix, `6909891` provider-card layout (scope top / rates bottom). regression-check GREEN on `main`. Branch deletable. Decisions (owner): single reactive component, rate-gen look, NO time estimate, spinner fills the parse gap; US-only (frozen A-Z left on `RealTimeProgressIndicator`); + share the chunked-write MATH via one pure helper (don't force one signature — each surface keeps its own band).
- NEW `components/shared/ProgressIndicator.vue` (reactive props `progress`(0–110)/`totalRows?`/`rowsProcessed?`/`label?`); 3 US surfaces migrated; Analyzer dropped its imperative `setProgress`/`complete` ref API + time-estimate → reactive `uploadProgress` per slot (upload + hand-off, via a new `ingestHandoffRows` onProgress passthrough); deleted `RateGenProgressIndicator` + shared `UploadProgressIndicator`.
- Adjuster "starts at 80%" was pre-existing `us-rate-sheet.service` curve (parse 0→70 instant, write squeezed into 85→98) → rebalanced monotonic write-dominated.
- NEW `utils/upload-progress.ts` `reportWriteProgress(stored,total,{from,to})` (pure, 6 unit tests) — Analyzer 0→99, Adjuster 40→99, Studio 50→100 all route their real chunked write through it. Also fixed the **Studio CSV freeze-at-90** (approximated timer ramped to ~90 then stored silently → now timer = 0→50 transform, real store = 50→100).
- regression-check GREEN; `test:unit` 188 passed; no new TS errors. **Next: owner gut-checks all 3 surfaces (CSV+XLSX, both themes) → conductor merges to `main` (deploy stays held, ships with (a)).**

**✅ DEPLOYED 2026-05-29 (manual Coolify by owner): PROD now `7189ff0`, tagged `prod-2026-05-29` (pushed). Verified live: `/api/ping` 200 lergCount 450, `/` 200, `/admin` 200.** Shipped the whole held stack: xlsx upload/export (ADR-0010) + cross-module hand-off arc (ADR-0009) + unified progress indicators + provider-card layout. `origin/main` = `7189ff0`, in sync. (Prior prod `277f1ea`/`prod-2026-05-28b` superseded.) Only remaining xlsx follow-up: (c) PDF rebrand (emerald → Switchboard).
